...
func (c *checker) inferCall(n *parser.Call) Type {
	...
	case *parser.Name:
		...
		case "abs":
			if len(n.Args) != 1 {
				c.errorf("abs() expects exactly one argument")
				n.Typ = TypAny
				return TypAny
			}
			argT := StripSpacetime(StripLinear(c.inferExpr(n.Args[0])))
			if typesEqual(argT, TypFloat) {
				n.Typ = TypFloat
				return TypFloat
			}
			if typesEqual(argT, TypInt) {
				n.Typ = TypInt
				return TypInt
			}
			n.Typ = TypAny
		...
		case "mir_romanai_gguf_path":
			if len(n.Args) != 0 {
				c.errorf("mir_romanai_gguf_path() expects no arguments")
				n.Typ = TypStr
				return TypStr
			}
			n.Typ = TypStr
		...
		case "mir_romanai_prompt":
			if len(n.Args) != 0 {
				c.errorf("mir_romanai_prompt() expects no arguments")
				n.Typ = TypStr
				return TypStr
			}
			n.Typ = TypStr
		...
		case "mir_romanai_cli_model_path":
			if len(n.Args) != 0 {
				c.errorf("mir_romanai_cli_model_path() expects no arguments")
				n.Typ = TypStr
				return TypStr
			}
			n.Typ = TypStr
		...
		case "mir_romanai_gguf_manifest_log":
			if len(n.Args) != 1 {
				c.errorf("mir_romanai_gguf_manifest_log() expects one argument (filepath str)")
				n.Typ = TypInt
				return TypInt
			}
			c.inferExpr(n.Args[0])
		...
		case "mir_romanai_gguf_layout_load":
			if len(n.Args) != 1 {
				c.errorf("mir_romanai_gguf_layout_load() expects one argument (filepath str)")
				n.Typ = TypInt
				return TypInt
			}
			c.inferExpr(n.Args[0])
		...
		case "mir_romanai_gguf_layout_free":
			if len(n.Args) != 0 {
				c.errorf("mir_romanai_gguf_layout_free() expects no arguments")
				n.Typ = TypInt
				return TypInt
			}
			n.Typ = TypInt
		...
		case "mir_romanai_gguf_layout_tensor_count":
			if len(n.Args) != 0 {
				c.errorf("mir_romanai_gguf_layout_tensor_count() expects no arguments")
				n.Typ = TypInt
				return TypInt
			}
			n.Typ = TypInt
		...
		case "mir_romanai_gguf_layout_tensor_vec4_slots":
			if len(n.Args) != 1 {
				c.errorf("mir_romanai_gguf_layout_tensor_vec4_slots() expects one argument (tensor index int)")
				n.Typ = TypInt
				return TypInt
			}
			c.inferExpr(n.Args[0])
		...
		case "mir_romanai_gguf_layout_tensor_rawptr":
			if len(n.Args) != 1 {
				c.errorf("mir_romanai_gguf_layout_tensor_rawptr() expects one argument (tensor index int)")
				n.Typ = TypRawPtr
				return TypRawPtr
			}
			c.inferExpr(n.Args[0])
		...
		case "mir_romanai_gguf_layout_tensor_ggml_type":
			if len(n.Args) != 1 {
				c.errorf("mir_romanai_gguf_layout_tensor_ggml_type() expects one argument (tensor index int)")
				n.Typ = TypInt
				return TypInt
			}
			c.inferExpr(n.Args[0])
		...
		case "mir_romanai_gguf_tensor_quant_bridge_prepare":
			if len(n.Args) != 1 {
				c.errorf("mir_romanai_gguf_tensor_quant_bridge_prepare() expects one argument (tensor index int)")
				n.Typ = TypInt
				return TypInt
			}
			c.inferExpr(n.Args[0])
		...
		case "mir_romanai_gguf_tensor_quant_bridge_ptr":
			if len(n.Args) != 0 {
				c.errorf("mir_romanai_gguf_tensor_quant_bridge_ptr() expects no arguments")
				n.Typ = TypRawPtr
				return TypRawPtr
			}
			n.Typ = TypRawPtr
		...
		case "mir_romanai_input_line":
			if len(n.Args) != 1 {
				c.errorf("mir_romanai_input_line() expects one argument (prompt str)")
				n.Typ = TypStr
				return TypStr
			}
			c.inferExpr(n.Args[0])
		...
		case "mir_romanai_decode_graph_step":
			if len(n.Args) != 1 {
				c.errorf("mir_romanai_decode_graph_step() expects one argument (filepath str)")
				n.Typ = TypInt
				return TypInt
			}
			c.inferExpr(n.Args[0])
		...
		case "mir_romanai_lattice_coupling_step":
			if len(n.Args) != 1 {
				c.errorf("mir_romanai_lattice_coupling_step() expects one argument (filepath str)")
				n.Typ = TypInt
				return TypInt
			}
			c.inferExpr(n.Args[0])
		...
		case "mir_romanai_vocab_print":
			if len(n.Args) != 1 {
				c.errorf("mir_romanai_vocab_print() expects one argument (vocab index int)")
				n.Typ = TypNone
				return TypNone
			}
			c.inferExpr(n.Args[0])
...
func seedBuiltins(s *Scope) {
	...
	add("mir_romanai_gguf_path", &Symbol{Name: "mir_romanai_gguf_path", Kind: SymBuiltin, Type: &Callable{Params: nil, Result: TypStr}})
	add("mir_romanai_prompt", &Symbol{Name: "mir_romanai_prompt", Kind: SymBuiltin, Type: &Callable{Params: nil, Result: TypStr}})
	add("mir_romanai_cli_model_path", &Symbol{Name: "mir_romanai_cli_model_path", Kind: SymBuiltin, Type: &Callable{Params: nil, Result: TypStr}})
	add("mir_romanai_gguf_manifest_log", &Symbol{Name: "mir_romanai_gguf_manifest_log", Kind: SymBuiltin, Type: &Callable{Params: []Type{TypStr}, Result: TypInt}})
	add("mir_romanai_gguf_layout_load", &Symbol{Name: "mir_romanai_gguf_layout_load", Kind: SymBuiltin, Type: &Callable{Params: []Type{TypStr}, Result: TypInt}})
	add("mir_romanai_gguf_layout_free", &Symbol{Name: "mir_romanai_gguf_layout_free", Kind: SymBuiltin, Type: &Callable{Params: nil, Result: TypInt}})
	add("mir_romanai_gguf_layout_tensor_count", &Symbol{Name: "mir_romanai_gguf_layout_tensor_count", Kind: SymBuiltin, Type: &Callable{Params: nil, Result: TypInt}})
	add("mir_romanai_gguf_layout_tensor_vec4_slots", &Symbol{Name: "mir_romanai_gguf_layout_tensor_vec4_slots", Kind: SymBuiltin, Type: &Callable{Params: []Type{TypInt}, Result: TypInt}})
	add("mir_romanai_gguf_layout_tensor_rawptr", &Symbol{Name: "mir_romanai_gguf_layout_tensor_rawptr", Kind: SymBuiltin, Type: &Callable{Params: []Type{TypInt}, Result: TypRawPtr}})
	add("mir_romanai_gguf_layout_tensor_ggml_type", &Symbol{Name: "mir_romanai_gguf_layout_tensor_ggml_type", Kind: SymBuiltin, Type: &Callable{Params: []Type{TypInt}, Result: TypInt}})
	add("mir_romanai_gguf_tensor_quant_bridge_prepare", &Symbol{Name: "mir_romanai_gguf_tensor_quant_bridge_prepare", Kind: SymBuiltin, Type: &Callable{Params: []Type{TypInt}, Result: TypInt}})
	add("mir_romanai_gguf_tensor_quant_bridge_ptr", &Symbol{Name: "mir_romanai_gguf_tensor_quant_bridge_ptr", Kind: SymBuiltin, Type: &Callable{Params: nil, Result: TypRawPtr}})
	add("mir_romanai_input_line", &Symbol{Name: "mir_romanai_input_line", Kind: SymBuiltin, Type: &Callable{Params: []Type{TypStr}, Result: TypStr}})
	add("mir_romanai_decode_graph_step", &Symbol{Name: "mir_romanai_decode_graph_step", Kind: SymBuiltin, Type: &Callable{Params: []Type{TypStr}, Result: TypInt}})
	add("mir_romanai_lattice_coupling_step", &Symbol{Name: "mir_romanai_lattice_coupling_step", Kind: SymBuiltin, Type: &Callable{Params: []Type{TypStr}, Result: TypInt}})
	add("mir_romanai_vocab_print", &Symbol{Name: "mir_romanai_vocab_print", Kind: SymBuiltin, Type: &Callable{Params: []Type{TypInt}, Result: TypNone}})
