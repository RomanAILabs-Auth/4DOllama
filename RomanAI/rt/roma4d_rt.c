...
/* MIR lowers `for x in range(n):` with a call to range(n) even when the body is flattened;
 * the return value is unused today — provide a linkable symbol. */
...
/* Distinct symbol for R4D kernel: avoids LLVM/MIR aliasing bugs when multiple mir_* str calls appear in one main(). */
static char g_romanai_cli_model_buf[4096];

/* Host sets ROMANAI_CLI_MODEL to the .gguf path after `romanai run <file>` (native main has no argv in R4D). */
